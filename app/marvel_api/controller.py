...
def search(response):
    ...
    if type == None or type not in['character', 'comic']:
        #Devuelve el orden de todos los caracteres por nombre
        return 'No se encontró un tipo válido, enumerando todos los personajes',__list_all_characters()
    
    filter =json.get_or_error(data, 'filter')
    if type.lower() == 'character':
        return 'Buscando personajes', search_character(filter)
    
    return 'Buscando cómics', search_comic(filter)
